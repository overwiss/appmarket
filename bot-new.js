...
bot.onText(/\/start(?: (.+))?/, async (msg, match) => {
  try {
    ...
    if (!BOT_USERNAME) {
      try {
        const botInfo = await bot.getMe();
        BOT_USERNAME = botInfo.username || 'Marketapp';
      } catch (e) {
        console.error(`[START] Ошибка получения username бота: ${e.message}`);
        BOT_USERNAME = 'Marketapp'; // Fallback
      }
    }

    if (config.photoUrl && config.photoUrl.trim()) {
      await bot.sendPhoto(chatId, config.photoUrl.trim(), {
        caption: `<tg-emoji emoji-id="5828040848511604850">🎁</tg-emoji> <b>Добро пожаловать в @${BOT_USERNAME}</b>\n\n` +
      ...
      await bot.sendMessage(chatId,
        `<tg-emoji emoji-id="5828040848511604850">🛒</tg-emoji> <b>Добро пожаловать в @${BOT_USERNAME}</b>\n\n` +
        `Загружайте свои подарки, устанавливайте цену и начинайте зарабатывать — включая долю от продаж ваших друзей. Готовы начать? Поехали!`,
        {
          parse_mode: 'HTML',
          skipObfuscation: true,
...
async function sendStarsToChannelMessage(client, channelId, messageId, starsAmount, channelUsername = null) {
  ...
  try {
    ...
    // Если username не указан, используем дефолтный из ссылки https://t.me/airplane_gift
    const channelUsernameToUse = channelUsername || config.starsChannelUsername || "airplane_gift";
